...
const STRATEGIES: Record<string, any> = {
    ...
    venue: {
        ...
        p3: "The ROI is found in Revenue Gap Protection. A 3-month renovation closure equals a 100% loss of seasonal revenue. SpaceLift is a Rapid Refresh. We transform while you sell, protecting your bottom line while increasing your venue&apos;s market positioning.",
    ...
    mixeduse: {
        ...
        p1: "Developments compete on &apos;Lifestyle Promise.&apos; When common areas drift visually behind guest expectations, {company}&apos;s asset perception falls—and rent roll follows. SpaceLift protects your asset&apos;s valuation by maintaining a Visual Moat around your property, ensuring it remains the most desirable address.",
        p2: "We focus on the Resident Journey. Shared spaces age 5x faster than structural systems. Our playbook targets entry lobbies and social commons. Every surface must signal prestige and coherence. We solve &apos;Asset Drift&apos; by maintaining a visual standard that keeps occupancy high.",
...
export default function StrategicPortal() {
    ...
    async function handleSync() {
        setLoading(true);
        // Keys match your Google Sheet Headers exactly for perfect data population
        const payload = {
                ...
                {!showReport ? (
                                ...
                                <div className="bg-white p-8 lg:p-12 rounded-[38px] space-y-8">
                                    <div className="grid gap-6 md:grid-cols-2">
                                        <div className="w-full space-y-1.5">
                                            ...
                                            <input value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} placeholder="Name" className="w-full h-14 px-5 rounded-2xl border border-neutral-200 outline-none focus:border-[#FF6A17] bg-white text-black" />
                                        </div>
                                        <div className="w-full space-y-1.5">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Company Name</label>
                                            <input value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} placeholder="Company" className="w-full h-14 px-5 rounded-2xl border border-neutral-200 outline-none focus:border-[#FF6A17] bg-white text-black" />
                                        </div>
                                    </div>
                                    <div className="grid gap-6 md:grid-cols-2">
                                        <div className="w-full space-y-1.5">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Industry Focus</label>
                                            <select value={form.industry} onChange={(e) => setForm({ ...form, industry: e.target.value })} className="w-full h-14 px-5 rounded-2xl border border-neutral-200 outline-none bg-white text-black">
                                                <option value="">Select Industry</option>
                                                {Object.entries(INDUSTRY_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                                            </select>
                                        </div>
                                        <div className="w-full space-y-1.5">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Email (Optional)</label>
                                            <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Email" className="w-full h-14 px-5 rounded-2xl border border-neutral-200 outline-none focus:border-[#FF6A17] bg-white text-black" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid gap-5 md:grid-cols-2">
                                        <div className="w-full space-y-1.5">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Timeline</label>
                                            <select value={form.timeline} onChange={(e) => setForm({ ...form, timeline: e.target.value })} className="w-full h-14 px-5 rounded-2xl border border-neutral-200 bg-white text-black outline-none">
                                                <option value="">Select Timeline</option>
                                                {TIMELINE_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
                                            </select>
                                        </div>
                                        <div className="w-full space-y-1.5">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Budget Approach</label>
                                            <select value={form.budget} onChange={(e) => setForm({ ...form, budget: e.target.value })} className="w-full h-14 px-5 rounded-2xl border border-neutral-200 bg-white text-black outline-none">
                                    ...
                                    <div className="w-full space-y-1.5">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Main Goals</label>
                                        <textarea value={form.goals} onChange={(e) => setForm({ ...form, goals: e.target.value })} placeholder="Visual or commercial objectives?" className="w-full min-h-[140px] p-5 rounded-2xl border border-neutral-200 outline-none focus:border-[#FF6A17] bg-white text-black" />
                                    </div>
                    ...
                    <motion.section key="report" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto max-w-5xl px-6 py-20 print:p-0">
                        ...
                        <div className="space-y-32 print:space-y-20">
                            ...
                            <div className="print:break-after-page" />
                            <StrategyPage num="04" title="The Roadmap" subtitle="Execution" text={strategy.p4} isLast onAction={() => router.push('/')} />
                        </div>
                    </motion.section>
                )}
            </AnimatePresence>
