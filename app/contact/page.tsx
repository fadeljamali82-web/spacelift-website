...
import {
    ...
    TrendingUp, Zap, Target, Check, ChevronRight, BarChart3, Clock, MousePointer2
} from "lucide-react";
...
// --- ELITE STRATEGIC CONTENT ENGINE ---
const STRATEGIES: Record<string, any> = {
    ...
    retail: {
        ...
        p3: "Speed is your competitive moat. Traditional retail fit-outs are slow and produce tons of waste. SpaceLift is Sustainable Luxury. We refresh your entire aesthetic in 4 days with 75% less waste and zero operational downtime. By remaining 100% operational, {company} captures the 'Hidden Revenue' that competitors lose during their 3-month construction closures. Visual ROI is maximized because the spend is concentrated solely on the customer's line of sight.",
        p4: "The path to Experiential Retail starts with identifying your high-impact 'Money Zones.' We apply elite surfaces that pop on digital platforms, turning every customer into a brand ambassador. We don't just sell products; we sell the environment they live in. SpaceLift is the fastest way to turn a transactional floor into a high-performance stage.",
        stats: [15, 92]
    },
    healthcare: {
        theme: "Trust Protocol",
        p1: "In healthcare, the environment is the first interaction of care. Patients form impressions of clinical quality based on visual clarity and calm before they ever speak to a doctor. If {company} looks dated, the Trust Gap widens, increasing patient anxiety and devaluing your care standards. SpaceLift closes that gap by signaling elite care through elite aesthetics. {fullName}, we help you realize a space that matches the caliber of your practitioners.",
                            ...
                            <div className="rounded-[40px] bg-[#F3EDE8] p-2 shadow-2xl border border-black/5">
                                ...
                                <div className="bg-white p-8 lg:p-12 rounded-b-[38px] space-y-6">
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 flex items-center gap-2"><MousePointer2 className="w-3 h-3" /> Select Scope (Multiple)</label>
                                        <div className="grid grid-cols-2 gap-2">
                                            {SERVICES.map(s => (
                                                <button key={s} onClick={() => toggleService(s)} className={`px-4 py-3 rounded-xl text-[10px] font-bold text-left border transition-all ${selectedServices.includes(s) ? 'bg-[#FF6A17] border-[#FF6A17] text-white shadow-md' : 'bg-white border-neutral-200 text-neutral-400 hover:border-neutral-300'}`}>{s}</button>
                                            ))}
                                        </div>
                                    </div>
