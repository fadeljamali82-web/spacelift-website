...
// --- OPTIONS ---
const BUDGET_OPTIONS = ["Phased / Value-driven", "Standard CapEx", "Premium Upgrade", "Serious Asset Transformation"];
const TIMELINE_OPTIONS = ["Immediate", "Within 3 months", "3-6 months", "Evaluating / TBC"];
const ENVIRONMENT_OPTIONS = ["Main Entrance/Lobby", "Corridors", "Guest Rooms", "Collaboration Zones", "Social Areas", "Executive Suites"];
...
const STRATEGIES: Record<string, any> = {
    ...
    corporate: {
        ...
        p4: "Our roadmap moves from Utility to Gallery. We identify high-traffic zones and apply high-performance architectural layers that reduce visual noise and promote focus. Our rapid deployment means zero downtime for your operations. SpaceLift is the fastest path to a world-class headquarters.",
        stats: [40, 88]
    },
    retail: {
        ...
        p3: "Speed is your competitive moat. Traditional retail fit-outs are slow and produce tons of waste. SpaceLift is Sustainable Luxury. We refresh your entire aesthetic in 4 days with 75% less waste and zero operational downtime. By remaining 100% operational, {company} captures the revenue that competitors lose.",
        p4: "The path to Experiential Retail starts with identifying your high-impact &apos;Money Zones.&apos; We apply elite surfaces that pop on digital platforms, turning every customer into a brand ambassador. We don&apos;t just sell products; we sell the environment they live in.",
        stats: [15, 92]
    },
    healthcare: {
        theme: "Trust Protocol",
        p1: "In healthcare, the environment is the first interaction of care. Patients form impressions of clinical quality based on visual clarity before they ever speak to a doctor. If {company} looks dated, the Trust Gap widens, increasing patient anxiety and devaluing your standards. SpaceLift closes that gap by signaling elite care.",
        p2: "We specialize in Atmospheric Calm. High-stress environments require Visual Decompression. Our playbook replaces clinical surfaces with warm, self-healing natural textures. By signaling safety and modernity, we solve for &apos;Atmospheric Anxiety&apos; and improve patient satisfaction scores.",
        p3: "Construction in healthcare is usually a nightmare of hygiene disruption. SpaceLift works in low-traffic windows with zero dust and zero noise, ensuring your care continuity remains 100% operational. Modernizing via resurfacing costs 70% less per square foot than traditional renovation.",
    ...
    mixeduse: {
        ...
        p1: "Developments compete on &apos;Lifestyle Promise.&apos; When common areas drift visually behind guest expectations, {company}&apos;s asset perception falls—and rent roll follows. SpaceLift protects your asset&apos;s valuation by maintaining a Visual Moat around your property.",
        ...
        p4: "The Roadmap protects your prestige for the long term. We identify where resident impressions are formed, apply durable, elite architectural surfaces that can be updated as trends evolve, and ensure your property never feels its age.",
        stats: [50, 92]
    }
};
...
export default function SpaceLiftPortal() {
    ...
    const [form, setForm] = useState({ fullName: "", company: "", email: "", industry: "", timeline: "", budget: "", goals: "" });

    const toggleEnv = (item: string) => setSelectedEnvs(p => p.includes(item) ? p.filter(i => i !== item) : [...p, item]);
                ...
                {!showReport ? (
                            ...
                            <div className="rounded-[40px] bg-[#F3EDE8] p-2 shadow-2xl border border-black/5">
                                <div className="bg-white p-8 lg:p-12 rounded-[38px] space-y-8">
                                    ...
                                    <div className="grid gap-5 md:grid-cols-2">
                                        <div className="space-y-1.5"><label className="text-[10px] font-black uppercase text-neutral-400">Timeline</label><select value={form.timeline} onChange={(e) => setForm({ ...form, timeline: e.target.value })} className="w-full h-14 px-5 rounded-2xl border border-neutral-200 bg-white text-black">{TIMELINE_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
                                        <div className="space-y-1.5"><label className="text-[10px] font-black uppercase text-neutral-400">Budget</label><select value={form.budget} onChange={(e) => setForm({ ...form, budget: e.target.value })} className="w-full h-14 px-5 rounded-2xl border border-neutral-200 bg-white text-black">{BUDGET_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
                                    </div>
                                    <div className="space-y-1.5"><label className="text-[10px] font-black uppercase text-neutral-400">Goals</label><textarea value={form.goals} onChange={(e) => setForm({ ...form, goals: e.target.value })} placeholder="Visual objectives?" className="w-full min-h-[120px] p-5 rounded-2xl border border-neutral-200 outline-none focus:border-[#FF6A17] bg-white text-black" /></div>
                                    <button disabled={loading || !form.fullName || !form.industry} onClick={handleSync} className="w-full h-16 bg-[#FF6A17] text-white font-bold text-lg rounded-2xl transition-all disabled:opacity-30 hover:shadow-xl flex items-center justify-center py-5 active:scale-[0.98]">{loading ? <Clock className="w-5 h-5 animate-spin" /> : "Generate Bespoke Strategy"}</button>
                                </div>
                            </div>
                        </div>
                    </motion.section>
                    ...
                    <motion.section key="report" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto max-w-5xl px-6 py-20 print:p-0">
                        ...
                        <div className="space-y-32 print:space-y-20">
                            ...
                            <StrategyPage num="02" title="Tactical Playbook" subtitle="Strategy" text={strategy.p2} />
                            <div className="print:break-after-page" />
                            <StrategyPage num="03" title="Economic ROI" subtitle="Alignment" text={strategy.p3} />
                            <div className="print:break-after-page" />
                            <StrategyPage num="04" title="Transformation Roadmap" subtitle="Execution" text={strategy.p4} isLast onAction={() => router.push('/')} />
                        </div>
                    </motion.section>
                )}
            </AnimatePresence>
                ...
                {stats && (
                    <div className="p-10 rounded-[50px] bg-neutral-100 border border-neutral-200 flex flex-col justify-center gap-10 print:border-none print:p-0">
                        <div className="w-full">
                            <div className="flex justify-between text-[10px] font-black uppercase mb-3 text-black"><span>Current Asset perception</span><span>{stats[0]}%</span></div>
